...
```{r prob5}
BGA = data.frame(bwt=sort(CLEAN$bwt), gestation=sort(CLEAN$gestation), age=sort(CLEAN$age))
tail(BGA)
```
...
```{r prob9}
# THREE S'S
# It is centered at `r median(foo)` the spread is `r IQR(foo)` and 
mean(nonsmokers)
median(nonsmokers)
mean(smokers)
median(smokers)
...
> **Prob 9 answer:** Smoker birth weights are skewed slightly to the right, as the birth weight is slightly lowered.
