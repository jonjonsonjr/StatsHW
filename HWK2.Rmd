...
Consider the population {3, 6, 7, 9, 11, 14}. For samples of size 3 without replacement,
...
Let X1,X2, . . . , Xn be a random sample from some distribution and suppose Y = T (X1,X2, . . . , Xn) is a statistic. Suppose the sampling distribution of Y has pdf $f(y) = (3/8)y^2, 0 \leq y \leq 2.$ Find P(0 $\leq Y /leq$ 1/5).
...
Let $X_1, X_2, \ldots X_10 \overset{iid}\sim $ N(20, 82) and $Y_1, Y_2, \ldots Y_15 \overset{iid}\sim$ N(16, 72). Let W = Xhat + Yhat.
...
Let $X_1, X_2, \ldots , X_9$
$\tilde{i.i.d.} N(7, 3^2)$ and $Y1, Y2, . . . , Y12$
$\tilde{i.i.d.} N(10, 5^2)$. Let $W = \hat{X} - \hat{Y}$.
a. Give the exact sampling distribution of $W$.
...
**Prob 14(a) answer:** The exact sampling distribution of $W$ is given by $W \tilde{} N(-3, `r sqrt(3^2/9 + 5^2/12)`)$. <br>
**Prob 14(b) answer:** See plot above.<br>
**Prob 15(c) answer:** By our simulation, $P(W < -1.5) =$ `r mean(W < -1.5)`. The exact value for $P(W < -1.5)$ is `r pnorm(-1.5, -3, sqrt(3^2/9 + 5^2/12))`. The simulated value has `r (mean(W < -1.5) - pnorm(-1.5, -3, sqrt(3^2/9 + 5^2/12)))/ pnorm(-1.5, -3, sqrt(3^2/9 + 5^2/12)) * 100`% error.

# Problem 15

Let X_1, X_2, ..., X_n be a random sample from N(0, 1). Let W = X_1^2 + X_2^2 + ... + x_n^2
...
Let $X_1, X_2, \ldots X_20 \overset{iid}\sim $ Exp(2). Let X = sum [1,20] X_i.
...
Let X1,X2 ~ F with corresponding pdf f (x) = 2/x2, 1 $\leq$ x $\leq$ 2.
...
Since $F(x) = \int_{0}^{x}2/t^2\,dt = -2/x, 1\leq x \leq 2,$ it follows that 
$$f_{\textrm{max}}(x) = n\left(-2/x\right)^{n-1}2/x^2,  1\leq x \leq 2$$

```{r}
# f <- function(x){2/x^2}
fmax <- function(x){2*(-2/x)^(2-1) * 2/x^2}
curve(fmax, from=1,to=2)
integrate(fmax, 1, 2)
#curve(f, from=1, to=2)
